...
#ifdef MH_USE_CHIP_RX	// MHH:17/03/2023. Modified version in ac210_comms.c
...
extern RINGBUFF_T U0_txring, U0_rxring;
extern RINGBUFF_T U2_txring, U2_rxring;
extern RINGBUFF_T U3_txring, U3_rxring;
uint8_t U0_buffer[64];
//uint8_t U2_buffer[64];
uint8_t U3_buffer[256];
int U0_bytes,U0_full_count;
int U2_bytes,U2_full_count;
int U3_bytes,U3_full_count;


int U0_ix,U3_ix;
//extern RINGBUFF_T U0_rxring2,U3_rxring2;
extern bool Uart0_rx_tx_link;
void Chip_UART_RXIntHandlerRB(LPC_USART_T *pUART, RINGBUFF_T *pRB)
{
	/* New data will be ignored if data not popped in time */
//	U3_ix = 0;

	while (Chip_UART_ReadLineStatus(pUART) & UART_LSR_RDR)	// MHH:17/03/2023. Modify to allow linking...
	{
		uint8_t ch = Chip_UART_ReadByte(pUART);

		if(Uart0_rx_tx_link)	// MHH:16/08/2023. Trying to find problem with UL serial. Redirect input to output
		{
			if(pUART == LPC_UART0)
			{
				if (Chip_UART_SendRB(LPC_UART0, &U0_txring, &ch, 1) != 1) U0_full_count++;
				U0_bytes++;
				return;
			}
		}
...
//				RingBuffer_Insert(&U0_rxring2, &ch);

				U0_buffer[U0_ix++] = ch;
				U0_ix &= 63;
				if (Chip_UART_SendRB(LPC_UART3, &U3_txring, &ch, 1) != 1) U3_full_count++;
				U0_bytes++;
				if(Hublink_check_escape(ch))
				{
					U0_bytes = U0_bytes;	// For debug
				}
//				DPRINTF("[0:%2x]",ch);		// MHH:09/06/2025
			}
			else
			{
				if(pUART == LPC_UART3)
...
//					if(U3_bytes < 100)
					...
					//					RingBuffer_Insert(&U3_rxring2, &ch);
				}
//				DPRINTF("[3:%2x]",ch);		// MHH:09/06/2025
			}
		}
		else
		{
	...
	if(U3_ix > 0)
	{
		Chip_UART_SendRB(LPC_UART0, &U0_txring, U3_buffer, U3_ix);
	...
	if(Sig100_hub_link)
	{
		U0_bytes = Chip_UART_ReadRB(LPC_UART0, &U0_rxring2, U0_buffer, 64);
		if (U0_bytes > 0)
		{
			if (Chip_UART_SendRB(LPC_UART3, &U3_txring, U0_buffer, U0_bytes) != U0_bytes) U0_full_count++;
		}
		U3_bytes = Chip_UART_ReadRB(LPC_UART3, &U3_rxring2, U3_buffer, 64);
		if (U3_bytes > 0)
		{
			if (Chip_UART_SendRB(LPC_UART0, &U0_txring, U3_buffer, U3_bytes) != U3_bytes) U3_full_count++;
		}
	}
...
void Chip_UART_TXIntHandlerRB(LPC_USART_T *pUART, RINGBUFF_T *pRB)
{
	...
	while(true)
	{
		if((Chip_UART_ReadLineStatus(pUART) & UART_LSR_THRE) == 0) return;
		if(RingBuffer_Pop(pRB,&ch) == 0) return;
		Chip_UART_SendByte(pUART, ch);
	}
#endif
}
