...
const uint32_t OscRateIn = 12000000;		// Crystal is 12 Mhz
const uint32_t RTCOscRateIn = 32768;

//=================================================================================================
//-----------------------------------------------------------
int Board_pin_read(uint8_t port,uint8_t pin)
{
//	uint32_t port = (pin_def >> 8);
//	uint8_t pin = (pin_def & 0xff);
	bool state = Chip_GPIO_ReadPortBit(LPC_GPIO, port, pin);
	if(state) return 1;
	return 0;
}
...
//	Chip_UART_SetBaud(DEBUG_UART, 115200);
	Chip_UART_SetBaud(DEBUG_UART, 19200);	// MHH:09/08/2023
...
int Board_test_for_DS3231(void)
{
// Both I2C pins should be set up as read with pulldown.
// if DS3231 present then both pins should be pulled up.

	int sda1_pin = Board_pin_read(PIN_DEF2(AC210_RTC_SDA1_PIND));
	int scl1_pin = Board_pin_read(PIN_DEF2(AC210_RTC_SCL1_PIND));

//	int reset_pin = Board_pin_read(PIN_DEF2(AC210_RTC_RESET_PIND));	// Not reliable

	return (sda1_pin + scl1_pin);
}
...
void Board_I2C_Init(I2C_ID_T id)
{
	switch (id) {
	case I2C0:
		Chip_IOCON_PinMux(LPC_IOCON, PIN_DEF2(AC210_I2C0_DATA_PIND),  IOCON_MODE_INACT, IOCON_FUNC1);
		Chip_IOCON_PinMux(LPC_IOCON, PIN_DEF2(AC210_I2C0_CLOCK_PIND), IOCON_MODE_INACT, IOCON_FUNC1);
		Chip_IOCON_SetI2CPad(LPC_IOCON, I2CPADCFG_STD_MODE);
		break;

	case I2C1:
		Chip_IOCON_PinMux(LPC_IOCON, PIN_DEF2(AC210_RTC_SDA1_PIND), IOCON_MODE_INACT, IOCON_FUNC3);	//MHH:01/01/19. RTC:SDA1
		Chip_IOCON_PinMux(LPC_IOCON, PIN_DEF2(AC210_RTC_SCL1_PIND), IOCON_MODE_INACT, IOCON_FUNC3);	//MHH:01/01/19  RTC:SCL1
		Chip_IOCON_EnableOD(LPC_IOCON,PIN_DEF2(AC210_RTC_SDA1_PIND));
		Chip_IOCON_EnableOD(LPC_IOCON,PIN_DEF2(AC210_RTC_SCL1_PIND));
		break;
/*
	case I2C2:
		...
		break;
*/
	default:		// MHH: Get rid of compiler warning
		break;
	}
}
